...
RUN apt-get update && apt-get install --no-install-recommends -y gpg curl git make ca-certificates apt-transport-https && \
    curl -sSL https://deb.nodesource.com/gpgkey/nodesource-repo.gpg.key%7Cgpg --dearmor >/etc/apt/trusted.gpg.d/nodesource-key.gpg && \
...
COPY --from=build /app/@stellar/design-system-website/build/ /usr/share/nginx/html/
